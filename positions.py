...
def parse_btg_positions(src: str | Path) -> pd.DataFrame:
    ...
    out = pd.DataFrame({
        ...
        "liquidez": _safe_col(df0, _pick_existing(cols, "Liquidez", "Prazo Liquidez", "Prazo de Liquidez", "Liquidação", "Liquidacao"), "").astype(str).str.strip(),
        "cotizacao_resgate": _safe_col(df0, _pick_existing(cols, "Prazo Cotização", "Prazo Cotizacao", "Cotização", "Cotizacao"), "").astype(str).str.strip(),
        "liquidacao_resgate": _safe_col(df0, _pick_existing(cols, "Prazo Liquidação", "Prazo Liquidacao", "Liquidação", "Liquidacao"), "").astype(str).str.strip(),
